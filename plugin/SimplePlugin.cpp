...
/*
 * Constructor for Simple Plugin type.
 ...
 */
SimplePlugin::SimplePlugin(Library* source, const String& name, 
		const String& funcName,	const String& regexp, int chance,
		Language lang)
: Plugin(source, Simple, name, funcName, regexp, chance, lang)
, _callback(NULL)
{
...
/*
 * Simple Plugin specific invocation method.
 ...
 */
bool SimplePlugin::execute(const String& nick, const String& host,
 ...
 * 	   library or not.
 */
bool SimplePlugin::refreshCallback()
{
	_callback = (SimplePluginFunc)dlSymbol(_source->getHandle(), _funcName);
